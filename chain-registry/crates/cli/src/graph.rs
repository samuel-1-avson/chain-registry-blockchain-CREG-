...
pub async fn run(
    ...
    println!("{} Building dependency graph for {} (depth {})", "→".cyan(), canonical.bold(), depth);
    ...
    if high_risk > 0 {
        println!("  {} High risk:  {}", "⚠".yellow(), high_risk.to_string().yellow());
    }
...
async fn resolve_deps(
    ...
    let record = match client.get(&url)
    ...
    let deps: Vec<String> = record.as_ref()
    ...
    let node = PackageNode { canonical: canonical.to_string(), status, risk_score, deps: deps.clone() };
    graph.insert(canonical.to_string(), node);

    if cur_depth < max_depth {
        // Use a boxed future to allow recursion
        for dep in deps {
            Box::pin(resolve_deps(&dep, base, max_depth, cur_depth + 1, visited, graph)).await;
        }
    }
}
...
fn render_tree(
    ...
    let status = node.as_ref().map(|n| n.status.as_str()).unwrap_or("unknown");
    ...
    let cycle_note = if already { " (already shown)".dimmed().to_string() } else { String::new() };
