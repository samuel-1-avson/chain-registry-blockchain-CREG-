...
fn main() {
    ...
    let exit_code = tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async {
            ...
            if !matches!(sub, "install" | "package" | "verify" | "compile"
                         | "dependency:resolve" | "dependency:get") {
...
async fn run_verified_build(ecosystem: &str, args: &[String]) -> i32 {
    ...
    println!("[chain-registry] Checking {} Maven dependencies...", packages.len());
    ...
    for (group_id, artifact_id, version) in &packages {
        ...
        match resolver::resolve(&canonical, Some(ecosystem), None).await {
            Ok(v) if v.status.is_blocked() => {
                eprintln!("\x1b[31m✗ BLOCKED\x1b[0m  {}:{} — chain registry: REVOKED", group_id, artifact_id);
                blocked = true;
            }
            Ok(v) if !v.status.is_safe() => {
                eprintln!("\x1b[33m⚠ WARNING\x1b[0m  {}:{} {} — not chain-verified.", group_id, artifact_id, version);
            }
...
fn read_pom_dependencies() -> Vec<(String, String, String)> {
    ...
    for line in content.lines() {
        ...
        if line == "<dependency>" {
            in_dep = true;
            group.clear(); artifact.clear(); version.clear();
        } else if line == "</dependency>" && in_dep {
            if !group.is_empty() && !artifact.is_empty() {
                deps.push((group.clone(), artifact.clone(),
                    if version.is_empty() { "RELEASE".into() } else { version.clone() }));
            }
            in_dep = false;
        } else if in_dep {
            if let Some(v) = extract_xml_value(line, "groupId")    { group    = v; }
            if let Some(v) = extract_xml_value(line, "artifactId") { artifact = v; }
            if let Some(v) = extract_xml_value(line, "version")    { version  = v; }
        }
    }
    deps
}
...
async fn run_real(args: &[String]) -> i32 {
    ...
    let real = match which::which_all("mvn")
        .ok()
        .and_then(|mut it| { it.next(); it.next() })
    {
        Some(p) => p,
        None    => { eprintln!("chain-registry: real mvn not found"); return 127; }
    };
