...
impl FeatureExtractor {
    ...
    pub fn extract(ecosystem: &str, code: &str) -> Result<PackageFeatures, FeatureError> {
        ...
        match ecosystem {
            "npm" | "node" | "javascript" | "typescript" => {
                Self::extract_js_features(code)
            }
            "pypi" | "python" => {
                Self::extract_python_features(code)
            }
            "cargo" | "rust" => {
                Self::extract_rust_features(code)
            }
    ...
    pub fn extract_js_features(code: &str) -> Result<PackageFeatures, FeatureError> {
        ...
        features.code_lines = code.lines()
            .filter(|l| !l.trim().is_empty() && !l.trim().starts_with("//"))
            .count() as u32;
        features.comment_lines = code.lines()
            .filter(|l| l.trim().starts_with("//") || l.trim().starts_with("/*"))
            .count() as u32;
        features.blank_lines = code.lines()
