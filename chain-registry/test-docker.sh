...
test_prebuilt_dryrun() {
    ...
    if docker-compose -f docker-compose.prebuilt.yml up --dry-run > /dev/null 2>&1; then
...
test_context_size() {
    ...
    if [ $CONTEXT_SIZE_MB -lt 500 ]; then
        log_success "Build context is reasonable: ${CONTEXT_SIZE_MB}MB"
    else
        log_warn "Build context is large: ${CONTEXT_SIZE_MB}MB (check .dockerignore)"
    fi
}
...
main() {
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo ""
        echo "Next steps:"
        echo "  1. docker-compose -f docker-compose.prebuilt.yml up -d"
        echo "  2. Or: docker build -f Dockerfile.minimal -t creg:minimal ."
        exit 0
    else
