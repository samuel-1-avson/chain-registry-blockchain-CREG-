...
"@
    ...
    $criticalReceiver = "  - name: critical"
    if ($SlackWebhookUrl -or $PagerDutyRoutingKey) {
        if ($SlackWebhookUrl) { $criticalReceiver += "`n$slackYaml" }
        if ($PagerDutyRoutingKey) { $criticalReceiver += $criticalPagerDuty }
    }

    $defaultReceiver = "  - name: default"
    if ($SlackWebhookUrl) { $defaultReceiver += "`n$slackYaml" }

    $warningReceiver = "  - name: warning"
    if ($SlackWebhookUrl) { $warningReceiver += "`n$slackYaml" }
...
Write-AlertmanagerGcpConfig -OutPath $alertmanagerPath -SlackWebhookUrl $slackUrl -PagerDutyRoutingKey $pagerKey
if ($slackUrl) {
    Log "Wrote $alertmanagerPath (Slack webhook configured)"
} elseif ($pagerKey) {
    Log "Wrote $alertmanagerPath (PagerDuty routing key configured)"
} else {
    Log "Wrote $alertmanagerPath (no external receivers - set GCP_ALERT_SLACK_WEBHOOK_URL in hosting.env)"
}
