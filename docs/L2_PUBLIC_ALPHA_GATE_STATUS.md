...
| IPFS pinning + availability (IPFS-001/002) | **Partial** | `ipfs-pin-check.py`, `gcp/run-ipfs-pin-check.ps1`, `gcp/setup-ipfs-pin-cron.ps1`; hourly cron on edge VM; reports in `testnet/ipfs-pin-logs/` |
| LLM advisory boundary visible (LLM-002) | **Partial** | Explorer `PackageIntelligencePanel`, CLI `output.rs`; hub disclaimers land in separate hub-web PR |
