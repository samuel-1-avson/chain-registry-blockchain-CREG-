...
| Property | Value |
|----------|-------|
| **Type** | Byzantine Fault Tolerant (BFT) blockchain |
...
├──────────────────┼───────────────────────────────────────────────────┤
│ Slash            │ A validator was penalized for bad behavior        │
│                  │ Contains: validator ID, amount, reason            │
├──────────────────┼───────────────────────────────────────────────────┤
│ ValidatorJoin    │ A new validator joined the network with stake     │
│                  │ Contains: validator ID, pubkey, stake amount      │
├──────────────────┼───────────────────────────────────────────────────┤
│ ValidatorLeave   │ A validator exited (stake enters unbonding)       │
...
───────────────────────────────────────────────
  ...
  b. Checks publisher is staked (≥ 0.01 ETH in Staking.sol)
...
# Stake to publish (minimum 0.01 ETH)
creg stake --amount 0.01eth --key ~/.creg/publisher.pem
...
If a validator approves a malicious package:
  ...
  → Validator loses 10% of their 1 ETH stake = 0.1 ETH penalty
  → Slash pool distributed to honest validators (reward)
...
The math for an attacker:
  ...
  4 × 1 ETH = 4 ETH minimum bribe + risk of slash + risk of exposure
